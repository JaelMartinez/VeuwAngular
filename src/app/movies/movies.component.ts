...
export class MoviesComponent implements OnInit, AfterViewInit {
  ...
  async loadMoviesAndInitialize() {
    ...
    try {
      ...
      this.popularMovies = await this.fillMoviesArrayWithTrailers(
        'popularMovies'
      );
      this.actionMovies = await this.fillMoviesArrayWithTrailers('action');
      this.comedyMovies = await this.fillMoviesArrayWithTrailers('comedy');
      this.sciFiMovies = await this.fillMoviesArrayWithTrailers('sci-fi');
      this.dramaMovies = await this.fillMoviesArrayWithTrailers('drama');
    ...
    while (moviesWithTrailers.length < 12) {
      ...
      switch (category) {
        ...
        case 'popularMovies':
          ...
          break;
        case 'action':
          movies = await this.movieService.getActionMovies(page);
          break;
        case 'comedy':
          movies = await this.movieService.getComedyMovies(page);
          break;
        case 'sci-fi':
          movies = await this.movieService.getSciFiMovies(page);
          break;
        case 'drama':
          movies = await this.movieService.getDramaMovies(page);
          break;
        default:
          return [];
      }
